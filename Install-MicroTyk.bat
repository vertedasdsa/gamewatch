@echo off
REM ============================================================
REM  GameWatch Automatic Installer (ONE-CLICK)
...
echo.
echo ========================================
echo   GameWatch Installation (Automatic)
echo ========================================
echo.

REM Determine source directory
set "SCRIPTDIR=%~dp0"
if not exist "%SCRIPTDIR%GameWatch.ps1" (
  echo ERROR: GameWatch.ps1 not found in %SCRIPTDIR%
...
set "PSSCRIPT=%TEMP%\GameWatch-Install-Temp.ps1"
...
(
  ...
  echo Write-Host "GameWatch Installation" -ForegroundColor Cyan
  echo Write-Host "========================================" -ForegroundColor Cyan
  echo Write-Host ""
  echo.
  echo # Paths
  echo $SourceDir = "%SCRIPTDIR%"
  echo $InstallDir = "C:\GameWatch"
  echo $DataDir = "$env:LOCALAPPDATA\GameWatch"
  echo.
  ...
  echo $files = @("GameWatch.ps1", "GameWatch.ps1", "Watchdog.ps1", "GameWatch.vbs", "config.ps1")
  ...
  echo Copy-Item (Join-Path $InstallDir "GameWatch.vbs") (Join-Path $startupDir "GameWatch.vbs") -Force
  echo Write-Host "  ✅ Done" -ForegroundColor Green
  echo Write-Host ""
  echo.
  ...
  echo.
  echo # GameWatch task
  echo $taskName = "GameWatch"
  echo $scriptPath = Join-Path $InstallDir "GameWatch.ps1"
  ...
  echo Write-Host "    ✅ GameWatch task" -ForegroundColor Green
  echo.
  echo # Watchdog task
  echo $watchdogName = "GameWatch_Watchdog"
  ...
  echo Write-Host ""
  echo.
  echo # Step 6: Start GameWatch
  echo Write-Host "[6/6] Starting GameWatch..." -ForegroundColor Yellow
  ...
  echo Write-Host "  1. Edit C:\GameWatch\config.ps1 with your Telegram Token + ChatId" -ForegroundColor Gray
  echo Write-Host "  2. Restart GameWatch (or wait for next login)" -ForegroundColor Gray
  echo Write-Host "  3. Launch a game - screenshot will arrive in Telegram" -ForegroundColor Gray
  echo Write-Host ""
  echo Write-Host "Config file: C:\GameWatch\config.ps1" -ForegroundColor Yellow
